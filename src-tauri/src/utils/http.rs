...
pub fn google_api_headers(access_token: &str) -> HeaderMap {
    ...
    h.insert(
        ...
        HeaderValue::from_str(ua)
            .unwrap_or_else(|_| HeaderValue::from_static("google-api-nodejs-client/10.3.0")),
    ); // [OPSEC] Wektor T Fallback CleanUp
       // [OPSEC Fix] Canonical MITM capture shows gl-node/22.21.1 (not 20.18.0)
    h.insert(
        HeaderName::from_static("x-goog-api-client"),
        HeaderValue::from_static("gl-node/22.21.1"),
    );
    h.insert(
...
pub fn go_ls_api_headers(access_token: &str) -> HeaderMap {
    ...
    h.insert(
        HeaderName::from_static("user-agent"),
        HeaderValue::from_str(&ua_str)
            .unwrap_or_else(|_| HeaderValue::from_static("cloudcode/1.22.2 windows/amd64")),
    );
...
pub fn google_oauth_headers() -> HeaderMap {
    ...
    h.insert(
        HeaderName::from_static("connection"),
        HeaderValue::from_static("keep-alive"),
    ); // [OPSEC] Wektor O: keep-alive sync
    h
}
