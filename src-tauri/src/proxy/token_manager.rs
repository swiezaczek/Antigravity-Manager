...
impl TokenManager {
        ...
        for entry in entries {
            ...
            match self.load_single_account(&path).await {
                Ok(Some(token)) => {
                    ...
                    self.email_index
                        .insert(token.email.clone(), account_id.clone());
    ...
    pub async fn reload_account(&self, account_id: &str) -> Result<(), String> {
        ...
        match self.load_single_account(&path).await {
            Ok(Some(token)) => {
                ...
                self.email_index
                    .insert(token.email.clone(), account_id.to_string());
                            ...
                            Err(e) => {
                                ...
                                if e.contains("\"invalid_grant\"") || e.contains("invalid_grant") {
                                    self.disable_account(
                                        ...
                                        &format!(
                                            "invalid_grant: {}",
                                            crate::proxy::upstream::client::sanitize_error_for_log(
                                                &e
                                            )
                                        ),
                                    )
    ...
    pub async fn save_refreshed_token_silent(
        &self,
        account_id: &str,
        ...
        content["token"]["expires_in"] =
            serde_json::Value::Number(token_response.expires_in.into());
        content["token"]["expiry_timestamp"] =
            serde_json::Value::Number((new_now + token_response.expires_in).into());
